...
int main(int argc, char **argv) {
    ...
    ros::init(argc, argv, "morris_aima_results_node");
    /**
     * NodeHandle is the main access point to communications with the ROS system.
     * The first NodeHandle constructed will fully initialize this node, and the last
     * NodeHandle destructed will close down the node.
     */
    ros::NodeHandle n("morris_aima_results");
    ...
    Result *results = ResultFactory::createResult(worldType, &n, properties);

    if(results==NULL) {
        ROS_INFO("ERROR: Unable to build Result object.");
        return -1;
    }

    ROS_INFO("Configuration complete. Initializing results module.");
    results->initialize();
    ROS_INFO("Results successfully Initialized!");
    ...
    while(ros::ok()) {
        ...
        ROS_INFO("waiting for environment info...");
        ros::spinOnce();
        loop_rate.sleep();
    }
